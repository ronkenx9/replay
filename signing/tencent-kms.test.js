...
function encodeDerSignature(r, s) {
    ...
    if (rBytes[0] !== undefined && rBytes[0] & 0x80)
        rBytes = Buffer.concat([Buffer.from([0x00]), rBytes]);
    if (sBytes[0] !== undefined && sBytes[0] & 0x80)
...
describe("Tencent KMS signer", () => {
    ...
    it("parses DER signatures into 32-byte r/s values", () => {
        const der = Buffer.from("3045022100e791e2b582ff977c61d5db283921762c4c81a28a2a2e7c61d5db283921762c4c02202c4c81a28a2a2e7c61d5db283921762c4c81a28a2a2e7c61d5db283921762c4c", "hex");
    ...
    it("determines the recovery id for a real secp256k1 signature", async () => {
        ...
        const v = await determineRecoveryId(hashMessage(message), `0x${signature.slice(2, 66)}`, `0x${signature.slice(66, 130)}`, account.address);
    ...
    it("calls Tencent Cloud KMS with the expected AsymmetricSign payload", async () => {
        ...
        const kmsSignature = await signer.account.signMessage({ message });
...
//# sourceMappingURL=tencent-kms.test.js.map
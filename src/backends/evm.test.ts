...
import { recordStep, verifyStep, type AgentStep } from "../core/recorder.js";
import { createEvmBackend, type EvmPublicClient, type EvmWalletClient } from "./evm.js";
...
function clients(): {
  ...
  const walletClient: EvmWalletClient = {
    ...
    async writeContract(request) {
      ...
      anchors.set(`${recorderAddress}:${String(runId)}:${String(seq)}`, packetHash as `0x${string}`);

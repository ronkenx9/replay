 ...
 */
function canonicalize(step, atMs) {
...
export async function recordStep(step, backend) {
 ...
 */
export async function verifyStep(receipt, backend) {
    let bytes;
    try {
    ...
    let recovered;
    try {
        recovered = JSON.parse(Buffer.from(bytes).toString("utf8"));
    }
    catch (err) {
        return {
            verified: false,
...
export async function verifyClaim(opts, backend) {
...
export async function recallRun(receipts, backend) {

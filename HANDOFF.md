...
> Last updated: 2026-06-09, post-audit session. Written to survive chat compaction: assumes
...
| Item | Proof |
|---|---|
| Phases 0–2 + most of P3 (TASKS.md) | 19/19 vitest, `npm run build` clean |
...
1. **P3 final box — public deploy.** Viewer + API to a public URL. Tencent serverless first
   ...
   **Gotcha:** viewer fetches hardcoded `http://localhost:4174` — make the API base
   configurable (`import.meta.env`) before deploying, or the public viewer silently falls
   back to static mock runs (that fallback is by design for offline/judges — keep it).
2. **P4 — MCP server.** REPLAY_RECORD / RECALL / VERIFY / FORK; copy the stdio pattern from
   `../gaslight/src/mcp-server.ts`. GATE: a Claude Code session calls RECALL on a real run.
3. **P4 — Tencent KMS signing** for the anchor wallet (port `../gaslight/src/submitter/kms.ts`,
   private-key fallback). CUT if behind.
4. **P5 — submission hardening. PROTECTED, never cut.**
...
Tencent KMS → MCP FORK action → COS blob storage.
...
```bash
npm run build && npm test        # tsc clean; 19/19 expected
...
- Codebase map: core `src/core/{recorder,evidence}.ts` · backends `src/backends/{memory,evm}.ts`
  ...
  · viewer `src/viewer/` (`viewer-data.ts` = static fallback runs) · contract

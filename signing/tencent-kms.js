...
import { hashMessage, hashTypedData, keccak256, recoverAddress, serializeSignature, serializeTransaction, } from "viem";
...
export function parseDerSignature(der) {
    ...
    if (lengthByte & 0x80)
        pos += lengthByte & 0x7f;
    if (der[pos] !== 0x02)
        throw new Error("Invalid R header in DER");
    const lenR = der[pos + 1];
    if (lenR === undefined)
        throw new Error("Invalid R length in DER");
    let rBytes = der.slice(pos + 2, pos + 2 + lenR);
    if (rBytes[0] === 0x00)
        rBytes = rBytes.slice(1);
    pos = pos + 2 + lenR;
    if (der[pos] !== 0x02)
        throw new Error("Invalid S header in DER");
    const lenS = der[pos + 1];
    if (lenS === undefined)
        throw new Error("Invalid S length in DER");
...
export async function determineRecoveryId(hash, r, s, expectedAddress) {
...
export function createTencentKmsAccount(config) {
...
async function signHashWithKms(hash, expectedAddress, config) {
    const { keyId, region = "ap-guangzhou", secretId, secretKey } = config;
    if (!keyId || !secretId || !secretKey)
        throw new Error("Tencent KMS credentials are incomplete.");
    ...
    if (data.Response?.Error)
        throw new Error(`Tencent Cloud KMS API Error: ${data.Response.Error.Message}`);
    if (!data.Response?.Signature)
        throw new Error("Tencent Cloud KMS returned no signature.");
...
export function buildTencentCloudHeaders(cfg) {
...
//# sourceMappingURL=tencent-kms.js.map
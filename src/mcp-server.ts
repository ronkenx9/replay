...
export function createReplayMcpServer() {
  ...
  server.registerTool(
    "REPLAY_RECORD",
    {
      ...
      description:
        "List locally recorded REPLAY runs. v1 recording still happens through the SDK; this MCP tool exposes the recorded blackbox artifacts.",
      inputSchema: {
        packetDir: z.string().optional().describe("Optional packet directory; defaults to data/packets."),
      },
    },
  ...
  server.registerTool(
    "REPLAY_FORK",
    {
      ...
      description:
        "Run the deterministic v1 fork model for a changed risk threshold and wallet idle amount. This does not claim LLM or eth_call replay.",
      inputSchema: {
